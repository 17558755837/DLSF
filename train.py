...
from models.M4D import M4D
import utils
from optim import create_optimizer
from scheduler import create_scheduler
from models.vit import interpolate_pos_embed
from tools.PrecisionMeter import AveragePrecisionMeter
...
def train(args, model, data_loader, optimizer, tokenizer, epoch, warmup_steps, device, scheduler,
    ...
    metric_logger.add_meter('loss_BIC2ti', utils.SmoothedValue(window_size=50, fmt='{value:.4f}'))
    ...
    metric_logger.add_meter('loss_con', utils.SmoothedValue(window_size=50, fmt='{value:.4f}'))
    ...
    for i, (text, image, fake_text_pos_list, fake_cls,  video, audio,fake_image_box,W, H,con_label_list) in enumerate(
        ...
        con_label_list = con_label_list.to(device)
        text_input = tokenizer(text, max_length=128, truncation=True, add_special_tokens=True,
                               return_attention_mask=True, return_token_type_ids=False)
        text_input, fake_token_pos = text_input_adjust(text_input, fake_text_pos_list, device)
        ...
        loss_MAC,loss_BIC2ti,loss_MLC,loss_TMG,loss_bbox, loss_giou,loss_con = model(text_input,image,video,audio,fake_cls,fake_token_pos,fake_image_box,con_label_list,alpha=alpha)


        loss = config['loss_MAC_wgt'] * loss_MAC \
               + config['loss_BIC2ti_wgt'] * loss_BIC2ti \
               + config['loss_bbox_wgt'] * loss_bbox \
               + config['loss_giou_wgt'] * loss_giou \
               + config['loss_TMG_wgt'] * loss_TMG \
               + config['loss_MLC_wgt'] * loss_MLC \
               + config['loss_con_wgt'] * loss_con

        loss.backward()
        optimizer.step()

        metric_logger.update(loss_MAC=loss_MAC.item())
        metric_logger.update(loss_BIC2ti=loss_BIC2ti.item())
...
def evaluation(args, model, data_loader, tokenizer, device, config):
    ...
    for i, (text, image, fake_text_pos_list, label, video, audio,fake_image_box,W, H,con_label_list) in enumerate(
            metric_logger.log_every(args, data_loader, print_freq, header)):
        ...
        logits_real_fake, logits_multicls, output_coord, logits_tok, it2va_attn_weight, image_attn_weight = model(text_input,image,video,audio,label,fake_token_pos,fake_image_box,is_train=False)
        # print(logits_real_fake.shape)
        # print(logits_multicls.shape)
        # print(logits_tok.shape)
...
def main_worker(args, config):
    ...
    start_epoch = 0
    max_epoch = config['schedular']['epochs'] #20
    ...
    model = M4D(args=args, config=config, text_encoder=args.text_encoder, tokenizer=tokenizer, init_deit=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--text_encoder', default='./bert-chinese')
    parser.add_argument('--output_dir', default='results')
    parser.add_argument('--device', default='cuda')
    parser.add_argument('--log', default=True)
    parser.add_argument('--token_momentum', default=True, action='store_true')
    parser.add_argument('--log_num', default='-l', type=str)
